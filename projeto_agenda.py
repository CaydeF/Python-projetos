...
def verifica_nome_repetido(nome):
    return pesquisa(nome) is not None

def novo():
    global agenda, alterada
    nome = pede_nome()
    if verifica_nome_repetido(nome):
        print(f"Erro: O nome '{nome}' já está na agenda.")
        return
...
def altera():
    ...
    if p is not None:
        ...
        if verifica_nome_repetido(nome) and nome != agenda[p][0]:
            print(f"Erro: O nome '{nome}' já está na agenda.")
            return
